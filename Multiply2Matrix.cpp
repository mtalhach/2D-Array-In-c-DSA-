// C++ Program to Multiply Two Matrix Using single Multi-dimensional Arrays
#include <iostream>
using namespace std;
#define rows 10
#define columns 10
...
public:
    ...
    void Multiplication(int r1, int c1)
    {
        for (int i = 0; i < r1; i++)
        {
            for (int j = 0; j < c1; j++)
        ...
        for (int i = 0; i < r1; i++)
        {
            for (int j = 0; j < c2; j++)
            {
                for (int k = 0; k < c1; k++)
                {
                    mul[i][j] += Matrix[i][k] * Matrix[k][j + c1];
                } // end k loop
            }     // end inner for loop
        }         // end outer for loop
                    ...
                    if (obj.r2 > 0 && obj.c2 > 0)
                    {
                        if (obj.r2 < 10 && obj.c2 < 10)
                        {
                            if (obj.r2 == obj.c2)
                            {
                                ...
                                obj.Multiplication(obj.r1, obj.c1);
